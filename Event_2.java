...
public class Event_2 {
    ...
    ps.setInt(1, 1);  // ID của sự kiện là 1
    ...
    if (rs.next()) {
        ...
        // Kiểm tra xem trường "list_mam_ngu_qua" có tồn tại không
        if (jsob.get("list_mam_ngu_qua") == null) {
            // Nếu chưa có, tạo mới trường "list_mam_ngu_qua"
            JSONArray jsar_1 = new JSONArray();
            jsob.put("list_mam_ngu_qua", jsar_1);
            }
        ...
        ps.setInt(2, 1);  // ID của sự kiện là 1
        ps.executeUpdate();
        ps.close();
         } else {
        ...
        jsob = new JSONObject();
        JSONArray jsar_1 = new JSONArray();
        jsob.put("list_mam_ngu_qua", jsar_1);  // Tạo mới trường list_mam_ngu_qua
    ...
    ps = conn.prepareStatement("UPDATE `event` SET `data` = ? WHERE `id` = ?;");
    ps.clearParameters();
    ps.setNString(1, Event_2.SaveData().toJSONString());  // Cập nhật dữ liệu sự kiện từ SaveData
    ps.setInt(2, 1);  // ID của sự kiện là 1
