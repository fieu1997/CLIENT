...
//        }
    ...
    public static void MainAttack(Map map, MainObject ObjAtk, MainObject focus, int idxSkill, LvSkill temp, int type) throws IOException {
        ...
        if (ObjAtk.isPlayer() && focus.isPlayer() && !map.isMapChiemThanh()
                && ((map.ismaplang && p.conn.ac_admin <= 10) || ObjAtk.level < 11 || focus.level < 11
        ...
        if (focus.isdie || focus.hp <= 0 && ObjAtk.isPlayer()) {
            ...
            }/* else if (focus instanceof Squire) {
            Squire squire = (Squire) focus;
            Squire.squireLeaveMap(squire.owner); // Xử lý khi Squire chết
        } */ else {
        ...
        if (p != null) {
            ...
            if (idxSkill == 0 && p.skill_110[0] >= 1 && p.skill_110[1] >= 1) {
...
//                dame += (long)(dame * 0.5);
        ...
        if (ObjAtk.isbolt()) {
            ptCrit += ObjAtk.get_CritDMG() * 0.0001;
        }
        //</editor-fold>

        dame += dame * DamePlus;

        long def = focus.get_DefBase();
...
//        }
        ...
        if (ObjAtk.isPlayer() && focus.isMob()) {
            ...
            boolean check = dame < 0
                    || (focus.isBoss() && Math.abs(focus.level - ObjAtk.level) >= 6 && focus.level < 120 && focus.template.mob_id != 174 && !Map.is_map_cant_save_site(focus.map_id));
...
//                    || (focus.isBoss() && focus.template.mob_id == 192 && map.zone_id == 3 && ObjAtk.level < 100);
        ...
        // 
        if (ObjAtk.get_Pierce() > ObjAtk.get_Crit() * 1.01) { // Xuyên giáp hơn chí mạng 1%
            if (xuyengiap) {
                dame += dame * (ptxuyengiap + 0.4);
                if (dame > 2_000_000_000) {
                    dame = 2_000_000_000;
                }
                ListEf.add(new Eff_TextFire(1, (int) dame)); // Hiệu ứng xuyên giáp
            }
        } else if (ObjAtk.get_Crit() > ObjAtk.get_Pierce() * 1.01) { // Chí mạng hơn xuyên giáp 1%
            if (ObjAtk.get_Crit() > Util.random(10_000)) {
                dame += dame * (ptCrit + 1);
                if (dame > 2_000_000_000) {
                    dame = 2_000_000_000;
                }
                ListEf.add(new Eff_TextFire(4, (int) dame)); // Hiệu ứng chí mạng
            }
        }
...
// dame Skill 110 tăng thêm
/*if (ObjAtk.isPlayer() && (idxSkill == 19 || idxSkill == 20)) {
        Player player = (Player) ObjAtk;

        // Tìm chỉ số tương ứng từ skill_110
        int skillUpgradeIndex = idxSkill - 19; // idxSkill 19 -> 0, idxSkill 20 -> 1
        if (player.skill_110[skillUpgradeIndex] > 0) {
            int bonusDamage = 800 + player.skill_110[skillUpgradeIndex] * 300;
            dame += bonusDamage;  // Cộng thêm sát thương từ nâng cấp skill_110
        }
    }*/

        if (ObjAtk.isPlayer() && (focus.isMob() || focus.isPlayer())) {
            damePhu = 0;
            idPlus = 12;
            if ((damePhu = p.getDameBt()) > 0) {
                idPlus = 11;
            } else if ((damePhu = p.getDameAs()) > 0) {
                idPlus = 10;
            }
            if (map.map_id == 124) {
                damePhu = 1;
        ...
        if (focus.hp <= 0) {
            if (focus.isPlayer() && ptHP > 70) {
                focus.hp = 5;
            } else if (focus.isbolt()) {
                map.lt.Updatesetwin(p);
            } else if (focus.isPlayer() && map.map_id == 46) {
                map.lt.Updatesetlose(p);
            } else {
...
//            }
        ...
        if (ObjAtk.isPlayer() && (focus.isPlayer() || focus.get_TypeObj() == 0)) {
            ...
            if (map.isMapChienTruong()) {
                ...
                MapService.Fire_Mob(map, p.conn, idxSkill, focus.index, (int) dame, focus.hp, ListEf, focus.template.mob_id, idPlus, damePhu);

            } else {
                
                //MapService.Fire_Mob(map, p.conn, idxSkill, focus.index, (int) dame, focus.hp, ListEf, 0, idPlus, damePhu);
               MapService.Fire_Mob(map, ObjAtk, idxSkill, focus.index, (int) dame, focus.hp, ListEf, 0, idPlus, damePhu);
...
//        }
        ...
        } else if (focus.isMob() && focus.isExp && ObjAtk.isPlayer()) {
            int expup = 0;
            expup = (int) dame; // tinh exp

            // Tính toán hệ số nhân EXP dựa trên chênh lệch cấp độ
            double expMultiplier = 1.0;
            int levelDiff = Math.abs(focus.level - p.level);// kiểm tra cấp độ 
          
            if (levelDiff <= 1) {
                expMultiplier = 1.0;
            } else if (levelDiff <= 5) {
                expMultiplier = 1.0 - (levelDiff - 1) * 0.1; // Giảm EXP 10% cho mỗi cấp độ chênh lệch
            } else {
                expMultiplier = 0.5; // Hệ số EXP tối thiểu cho chênh lệch cấp độ cao hơn
            }

            // Áp dụng hệ số EXP và các điều chỉnh khác
            expup = (int) (expup * expMultiplier);

            // Nhân 3 EXP cho người chơi cấp độ 20 trở xuống
            if (p.level <= 20) {
                expup *= 3;
            }

            // Giảm một nửa EXP nếu người chơi bị suy yếu
            if (p.hieuchien > 0) {
                expup /= 2;
            }

            if (levelDiff <= 10 && expup > 0) {
                if (p.party != null) {
                ...
                p.update_Exp(expup, true);
            } else if(p.level >= 145) { // nếu 145 bỏ qua giới hạn exp
              ...
              if (p.party != null) {
                ...
                // Lấy hiệu ứng với ID -125
                ef = p.get_EffDefault(-125);

                // Kiểm tra nếu ef không bị null
                if (ef != null) {
                    if (ef.param == 5000) {
                        p.update_Exp(2, false);
                    } else if (ef.param == 10000) {
                        p.update_Exp(4, false);
                    } else {
                        p.update_Exp(1, false);
                    }
                } else {
                    // Trường hợp ef bị null, cập nhật exp mặc định
                    p.update_Exp(1, false);
                    //System.out.println("Hiệu ứng ef bị null, áp dụng cập nhật exp mặc định.");
                }
            }
            ...
            if (!focus.isdie && p.pet_follow != -1) {
                ...
                if (my_pet != null && my_pet.grown > 0) {
                    ...
                    if (((focus.hp - dame_pet) > 0) && (p.pet_atk_speed < System.currentTimeMillis()) && (a2 > 1)) {
                        ...
                        if (focus.isPlayer() && my_pet.get_id() == 4626 && Util.nextInt(100) < 100) {
                            Player p_focus = (Player) focus;
    ...
    public int changeDame(int dame) {
        return this.dame += dame; // Trả về giá trị mới của dame
    }

    public int changeDame_(int dame) {
        return this.dame -= dame; // Trả về giá trị mới của dame
    }
