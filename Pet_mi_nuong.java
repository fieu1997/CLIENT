...
public class Pet_mi_nuong extends MainObject {
    ...
    public Pet_mi_nuong(int type, int index_mob, int x, int y, int id_map, String name, Player p) {
    ...
    public void SetDie(Map map, MainObject mainAtk) {
        try {
            if (this.hp <= 0) {
                this.isdie = true;
                this.hp = 0;
                Pet_mi_nuong_manager.remove(this.name);
                this.p.pet_mi_nuong = null;
    ...
    public static boolean checkMy(Session conn) throws IOException {
        if (conn.p.pet_mi_nuong != null) {
            Service.send_notice_box(conn, "Đang dắt Mỵ Không Thể Thực hiện ");
            return true;
        }
        return false;
    }
