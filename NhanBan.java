...
public class NhanBan extends MainObject {
    ...
    private Timer respawnTimer; // Khai báo timer bên ngoài hàm
    private boolean respawnTimerRunning = false; // Theo dõi trạng thái của timer
        ...
        try {
            if (this.hp <= 0) {
                ...
                if (temp_mob != null) {
                    ...
                    // Khởi động timer nếu nó chưa chạy
                        ...
                        respawnTimer.schedule(new TimerTask() {
                            ...
                            public void run() {
                                try {
                                    ...
                                    temp_mob.nhanban.hp = temp_mob.nhanban.get_HpMax();
                                    temp_mob.nhanban.x = temp_mob.x; 
                                    temp_mob.nhanban.y = temp_mob.y; 
                                ...
                                for (int i = 0; i < map.players.size(); i++) {
                                    ...
                                    if (p0.map.map_id == map.map_id) {
                                        p0.conn.addmsg(m12);
                                    }
                                }
                                m12.cleanup();

                                    respawnTimerRunning = false; // Đặt lại trạng thái của timer
                                } catch (IOException e) {
                                    e.printStackTrace();
                                }
                            }
                        }, 10_000); // 10 giây
                        respawnTimerRunning = true; // Đánh dấu timer đang chạy
                    }
                }
            }
        } catch (Exception e) {
