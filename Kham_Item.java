...
public class Kham_Item {
    ...
    public static boolean CheckNgocTinhLuyen(short id) {
        if ((id >= 382 && id <= 386) || (id >= 387 && id <= 391) ||(id >= 392 && id <= 396) || (id >= 397 && id <= 401) || (id >= 402 && id <= 406) ||(id >= 407 && id <= 411))// hỗn nguyên tinh luyện
            return true;
            if (id >= 23 && id <= 32) {
            return true; // Ngọc từ 23 đến 32 không cần tinh luyện
        }
        return false;
    }
    ...
    private static Option GetOps(short id, short idItem) {
        Option re = null;
        // Tùy chọn đặc biệt cho ngọc từ ID 23 đến 32
        if (id >= 23 && id <= 27) { // Ngọc Ánh Sáng
        int param = (int) Math.pow(2, (id - 23)) * 100;  // Param theo lũy thừa 2: 100, 200, 400, 800, 1600
        int optionId = 6;  // Option ID cho ngọc Ánh Sáng là 6
        re = new Option(optionId, param, idItem);
    } 
    // Tùy chọn đặc biệt cho ngọc từ ID 28 đến 32 (Bóng Tối)
    else if (id >= 28 && id <= 32) { // Ngọc Bóng Tối
        int param = (int) Math.pow(2, (id - 28)) * 100;  // Param theo lũy thừa 2: 100, 200, 400, 800, 1600
        int optionId = 5;  // Option ID cho ngọc Bóng Tối là 5
        re = new Option(optionId, param, idItem);
    }
        else if (id >= 352 && id <= 356)// hỗn nguyên
        {
            int pr = (357 - id) * 5;
