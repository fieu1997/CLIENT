...
//                }
    ...
    public static void saveEventData(Connection conn) throws SQLException {
        // Chuẩn bị truy vấn dữ liệu sự kiện với ID 0
        PreparedStatement ps = conn.prepareStatement("SELECT `data` FROM `event` WHERE `id` = ?;");
        ps.setInt(1, 0);  // ID của sự kiện là 0
        ...
        if (rs.next()) {
            ...
            // Kiểm tra xem trường "list_naukeo" có tồn tại không
            if (jsob.get("list_naukeo") == null) {
                // Nếu chưa có, tạo mới trường "list_naukeo"
                JSONArray jsar_1 = new JSONArray();
                jsob.put("list_naukeo", jsar_1);
              } 
            // Kiểm tra xem trường "list_caythong" có tồn tại không
            if (jsob.get("list_caythong") == null) {
                // Nếu chưa có, tạo mới trường "list_caythong"
                JSONArray jsar_2 = new JSONArray();
                jsob.put("list_caythong", jsar_2);
                }
            ...
            ps.setInt(2, 0);  // ID của sự kiện là 0
            ps.executeUpdate();
            ps.close();
             } else {
            // Nếu không tìm thấy sự kiện có ID là 0, tạo mới sự kiện
             jsob = new JSONObject();
            
            // Tạo mới các trường list_naukeo và list_caythong
            JSONArray jsar_1 = new JSONArray();
            jsob.put("list_naukeo", jsar_1);
            ...
            // Chèn dữ liệu mới vào bảng sự kiện
        ...
        rs.close();
        // Sau khi xử lý xong kiểm tra và cập nhật dữ liệu sự kiện từ SaveData
        ps = conn.prepareStatement("UPDATE `event` SET `data` = ? WHERE `id` = ?;");
        ps.clearParameters();
        ps.setNString(1, Event_1.SaveData().toJSONString());  // Cập nhật dữ liệu sự kiện từ SaveData
        ps.setInt(2, 0);  // ID của sự kiện là 0
        ps.executeUpdate();  // Thực thi lệnh cập nhật
        ps.close();  // Đóng kết nối.
    }

    // Hàm SaveData để lấy dữ liệu lưu trữ của sự kiện 1
