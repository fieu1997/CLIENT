...
public class GameSrc {
    ...
    private static boolean check_item_can_rebuild(byte type) {
        switch (type) {
            ...
            case 6: // shoes
            // case 7: // wing Cánh
            case 15: // fashion cải trang
            case 8:
            case 9:
            case 10:
            case 16: //mề đay
            case 21: // mặt nạ
            case 22: // cánh thời trang
            case 23:  // áo choàng
            case 24:   // vũ khí tb2
            case 25:   //tóc thời trang
            case 26:    // tai nghe 
            case 27:  // danh hiệu
            case 28:  //giáp siêu nhân
            case 29:   // long bảo
            case 30: // ngọc thạch
            case 11: {
                return true;
    ...
    public synchronized static void trade_process(Session conn, Message m2) throws IOException {
        ...
        if (!Manager.isGiaoDich) {
            Service.send_notice_box(conn, "Giao dịch đang tạm khóa!");
            return;
        }
        if (conn.ac_admin == 15) {
            Service.send_notice_box(conn, "Không thể gd!");
            return;
        }
        switch (type) {
            case 0: {
    try {
        ...
        if (targetName == null || targetName.trim().isEmpty()) {
            Service.send_notice_box(conn, "Tên nhân vật mục tiêu không hợp lệ!");
            return;
        }

        Player p0 = Map.get_player_by_name(targetName);
        if (p0 == null || p0.map != conn.p.map) {
            Service.send_notice_box(conn, "Nhân vật mục tiêu không ở gần bạn!");
            return;
        }

        // Kiểm tra khoảng cách
        if (Math.abs(p0.x - conn.p.x) >= 200 || Math.abs(p0.y - conn.p.y) >= 200) {
            Service.send_notice_box(conn, "Nhân vật mục tiêu quá xa để giao dịch!");
            return;
        }

        // Kiểm tra trạng thái bận
        if (p0.list_item_trade != null || (p0.name_trade != null && !p0.name_trade.isEmpty())) {
            Service.send_notice_box(conn, "Đối phương đang có giao dịch!");
            return;
        }

        // Kiểm tra trạng thái kết nối
        if (p0.conn == null || !p0.conn.connected) {
            Service.send_notice_box(conn, "Nhân vật mục tiêu không hoạt động!");
            return;
        }

        // Kiểm tra logic giao dịch dựa trên isOwner
        if (p0.isLiveSquire) { // p và p0 khác loại (Player vs Squire hoặc ngược lại)
            Service.send_notice_box(conn, "Chỉ có thể giao dịch với cùng loại nhân vật (Player với Player, Squire với Squire)!");
            return;
        }
        if (conn.p == p0) { // Không giao dịch với chính mình
            Service.send_notice_box(conn, "Không thể giao dịch với chính mình!");
            return;
        }
        ...
        Service.send_notice_box(conn, "Lỗi dữ liệu giao dịch, vui lòng thử lại!");
        e.printStackTrace();
    }
    break;
}
        ...
        if (actions == 94) {
            ...
            for (int i = 0; i < it_temp.op.size(); i++) {
                ...
                if (op != null && op.id >= 0 && op.id <= 4) {
                    ...
                    // _st = Util.random(600, 800);
                    // }
                    it_temp.op.set(i, new Option(Util.ngaunhien(0, 4), _st, it_temp.id));
        ...
        if (actions == 98) {
            ...
            for (int i = 0; i < it_temp.op.size(); i++) {
                ...
                if (op != null && op.id >= 7 && op.id <= 11) {
                    ...
                    // _st = Util.random(600, 800);
                    // }
                    it_temp.op.set(i, new Option(Util.ngaunhien(7, 11), _st, it_temp.id));
                    Service.send_wear(conn.p);
                    Service.send_notice_box(conn, "Thành công");
                    return;
                }
                ...
                if (conn.p.item.total_item_by_id(4, 135) > 0) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (p_store.it_type == 3) {
                            ...
                            if (p_store.it_type == 4 && (Helps.CheckItem.item4CanTrade(p_store.it_id) || p_store.it_id == 135 || p_store.it_id == 52 || p_store.it_id == 56 || p_store.it_id == 143 || p_store.it_id == 226 || p_store.it_id == 329)) {
...
//                System.out.println(id_g3);
                ...
                // Bỏ qua kiểm tra tinh luyện cho ngọc có ID từ 23 đến 32
                if (id_g1 < 23 || id_g1 > 32) {
...
// Nếu người chơi có quyền admin cao và đang bật BuffAdmin thì luôn có color = 5
if ((conn.ac_admin > 4 && Manager.BuffAdmin)) {
    color = 5;
} if (conn.vip >= 6) {
    // Phân bố cho người chơi VIP đã điều chỉnh:
    // Color 5: 10%, Color 4: 12%, Color 3: 20%, Color 2: 16%, Color 1: 19%, Color 0: 23%
    if (ran >= 90) {          // ran: 90..99 -> 10 số -> 10%
        color = 5;
    } else if (ran >= 78) {   // ran: 78..89 -> 12 số -> 12%
        color = 4;
    } else if (ran >= 58) {   // ran: 58..77 -> 20 số -> 20%
        color = 3;
    } else if (ran >= 42) {   // ran: 42..57 -> 16 số -> 16%
        color = 2;
    } else if (ran >= 23) {   // ran: 23..41 -> 19 số -> 19%
        color = 1;
    } else {                  // ran: 0..22  -> 23 số -> 23%
        color = 0;
    }
} else {
    // Phân bố cho người chơi không VIP:
    if (ran >= 95) {          // ran: 95..99 -> 5% 
        color = 5;
    } else if (ran >= 87) {   // ran: 87..94 -> 8%
        color = 4;
    } else if (ran >= 72) {   // ran: 72..86 -> 15%
        color = 3;
    } else if (ran >= 53) {   // ran: 53..71 -> 19%
        color = 2;
    } else if (ran >= 29) {   // ran: 29..52 -> 24%
        color = 1;
    } else {                  // ran: 0..28  -> 29%
        color = 0;
    }
...
//
                    ...
                    for (int i = 0; i < ops.size(); i++) {
                        ...
                        int pr1 = (int) +(pr + (int) +(pr + (color) + 0.167)/2);
                        int pr2 = (int) +(pr + (int) +(pr + (color + 1) + 0.167)/2);

                        if ((o.id >= 58 && o.id <= 61) || (o.id >= 100 && o.id <= 107)) {
                    ...
                    if (color == 4) {
                        if (itbag.type == 0 || itbag.type == 1) {
                            ...
                            if (percent > 85) {
                                ...
                                while (opid1 == opid2 || opid1 == opid3 || opid2 == opid3) {
...
//        try{
        ...
        for (int i = conn.p.TypeItemStarCreate * 5; i < conn.p.TypeItemStarCreate * 5 + 5; i++) {
            if (conn.p.item.total_item_by_id(7, conn.p.MaterialItemStar[i]) < 1 && (conn.ac_admin < 4 || !Manager.BuffAdminMaterial)) {
                Service.send_notice_box(conn, "Không đủ " + ItemTemplate7.item.get(conn.p.MaterialItemStar[i]).getName() + "!");
                return;
            }
        }
        ...
        if (suc) {
            ...
            temp.op.clear();
            temp.UpdateName();
            for (int i = 0; i < ops.size(); i++) {// nâng chỉ số tt
                Option o = ops.get(i);
                int pr = (int) o.getParam(0);
                int pr1 = (int) (pr + (int) (pr * (temp.color) * 0.167)/2);
                int pr2 = (int) (pr + (int) (pr * (temp.color + 1) * 0.167)/2);
                ...
                if ((o.id >= 58 && o.id <= 61) || (o.id >= 100 && o.id <= 107)) {
                    ...
                    if (!((o.id >= -128 && o.id <= -80) || (o.id == 99))) {
                        temp.op.add(new Option(o.id, Util.random(pr1, pr2), temp.id));
                    } else { // chỉ số op
                        int id_opt = o.id;
                        int opt_par = o.getParam(0);
                        switch (temp.tierStar) {
                            case 1, 2, 3, 4, 5, 6, 7, 8, 9:
                                opt_par += Util.random(99, 199);
                                temp.op.remove(o);
                                temp.op.add(new Option(id_opt, opt_par));
