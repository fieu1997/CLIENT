        ...
        this.server_live = new Thread(() -> {
            ...
            while (this.running) {
                try {
                    ...
                    checkError = 2;
                    if (min % 5 == 0 && sec == 0) {
                        Manager.gI().chatKTGprocess("Bạn Đang Chơi Server" + " Hiệp Sĩ online " + "Chúc Bạn Chơi Game Vui Vẻ.");
                    }
                    // Gọi hàm runNpc mỗi phút
                    if (min % 1 == 0 && sec == 0) {
                        //Pet_mi_nuong.ranNpc();
                    }
                    checkError = 5;
                    ...
                    if (Manager.gI().event == 5) {
                        if (Event_5.naubanh == null) {
                            Event_5.naubanh = new NauBanh();
                        }
                        Event_5.naubanh.h = hour;
                        Event_5.naubanh.m = min;
                        if (hour == 17 && min == 00 && sec == 00) {
                            Event_5.list_nhanbanh.clear();
                    ...
                    //TienTai
                    if (hour == 12 && min == 00 && sec == 0 || hour == 18 && min == 00 && sec == 0) {

                        ChienTruong1.gI().open_register();
                        Manager.gI().chatKTGprocess("Tiền Tài Vô Tận Đã Mở Nhanh Tay Đăng Ký Nào");
                    }
                    if (sec % 1 == 0) {
                        ChienTruong1.gI().update();
                    }
                    if (sec % 1 == 0) {// Reset Chặn ip
                        Session.refreshBlockedIPs();
                    }
