...
class HSOMapViewer(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("HSO Map Viewer - Test Edition")
    ...
    def update_objects_list(self):
        ...
        if self.map_data.decorative_icons:
            ...
            for i, icon in enumerate(self.map_data.decorative_icons):
                text += f"{i+1:2d}. ID:{icon['template_id']:3d} at ({icon['x']//24:2d}, {icon['y']//24:2d})\n"
            text += "\n"
        ...
        if self.map_data.internal_vgos:
            ...
            for i, vgo in enumerate(self.map_data.internal_vgos):
                text += f"{i+1:2d}. '{vgo['name']}' at ({vgo['x']//24:2d}, {vgo['y']//24:2d})\n"
            text += "\n"
        ...
        if self.map_data.effect_triggers:
            ...
            for i, trigger in enumerate(self.map_data.effect_triggers):
                text += f"{i+1:2d}. ID:{trigger['id']:2d} at ({trigger['x']//24:2d}, {trigger['y']//24:2d})\n"
...
def main():
    print("=== HSO Map Viewer - Test Edition ===")
    ...
    print("🚀 Starting HSO Map Viewer (Test Edition)...")
    print("📋 Features:")
