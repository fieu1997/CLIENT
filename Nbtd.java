...
public class Nbtd extends MainObject {
    ...
    public Player A;
    public Player B; // Biến đánh dấu chủ nhân
    public Mob_in_map mob_target;
    public long spawnTime;

    public Nbtd(Player A, Player B) {
        this.A = A;
        this.B = B;
        this.index = B.index;
        this.map_id = 36;
        this.zone_id = A.map.zone_id;
        this.x = A.x;
        this.y = A.y;
    }

    public Nbtd(int mapid, int id, String name, int x, int y, int clzz, int head, int eye, int hair, int lv, int hpMax, int pk, byte[] fashions, int mount, int matna,
    ...
    public void setup(Player p0) {
        //this.B = p0; // Gán chủ nhân cho Nbtd
        ...
        if (p0.myclan != null) {
            ...
            this.clan_name_clan_shorted = "Thách Đấu";
    ...
    public static void Fire_Mob(Map map, Nbtd nbtd, int indexskill, int idMobTarget, int damage, int hpMobTarget, List<Eff_TextFire> listFireEffects, int idPlus, int damagePlus) throws IOException {
    Message m = new Message(9);
    m.writer().writeShort(nbtd.index);
    ...
    m.writer().writeInt(nbtd.hp);
    m.writer().writeInt(nbtd.mp);
    m.writer().writeByte(idPlus);
    m.writer().writeInt(damagePlus);
    MapService.send_msg_player_inside(map, nbtd, m, true);
    m.cleanup();
}
