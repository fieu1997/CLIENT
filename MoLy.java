...
public class MoLy {
    ...
    public static void Lottery_process(Player p, Message m2) throws IOException {
        ...
        switch (step) {
            ...
            case 2: {
                Session conn = p.conn;
                if (conn.p.nv_tinh_tu[0] == 18 && conn.p.nv_tinh_tu[1] < conn.p.nv_tinh_tu[2]) {
                    conn.p.nv_tinh_tu[1]++;
                    Service.send_notice_nobox_white(p.conn, "Tiến Độ " + conn.p.nv_tinh_tu[1] + "/" + conn.p.nv_tinh_tu[2]);
                }
                if (p.id_select_mo_ly != -1) {
